...
export class PhramcyService {
  ...
  insertToMedicineIssue(patientinfo,medicineinfo,from,hprofile) {
    let datas = JSON.stringify({patientinfo:patientinfo,medicineinfo:medicineinfo,from:from,hprofile:hprofile});
    return new Promise(resolve => {
